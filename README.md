...
- 静态 HTML 页面返回
- 404 错误处理
- JSON API 端点，使用自研 JSON 库动态生成响应（目前仅在v0版本使用，v1、v1.5主要着力与优化服务器性能）
- 连接关闭
...
- V1（已完成）：epoll + 非阻塞 I/O + Reactor 模式——可接受5w+的并发，使用wrk进行压测的时候在5w并发的情况下达到了7w+的QPS以及200ms+的延迟
- v1.5 （已完成）：在v1的基础上将读写优化成使用响应队列进行响应处理，在5w并发的情况下同样200ms的平均延迟下QPS达到了8w+；同时v1.5的架构也更加工程化规范化
- V2（后续）：线程池 + 内存池 + 定时器
